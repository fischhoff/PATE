...
---
#316 one.per.var remove abs
##to do: update PATE_data.csv
...
yes = subset(out, include_article_review == "yes")
save(yes, file = "yes.Rdata")
```
...
```{r}
load("yes.Rdata")
load("S.Rdata")#qualitative
load("P.Rdata")#quantitative
...
##do funnel plot analysis
```{r}
load("M.Rdata")
M = subset(M, !is.na(d))
x = M$d
vi = M$d.sampling.variance
funnel(x, vi, yaxis = "vi", main="Sampling Variance")
M_nonabs = M
save(M_nonabs, file = "M_nonabs.Rdata")

# ranktest(x, vi)
res2 <- cor.test(x, vi,  method="kendall")
res2
## Default S3 method:
```

###change d to absolute d
```{r}
load("M.Rdata")
M$d = abs(M$d)
save(M, file = "M.Rdata")
...
stable.m$Pathway[stable.m$Pathway=="abund biomass to ecosystem fxn"]="abundance"
stable.m$Pathway[stable.m$Pathway=="morbidity to ecosystem fxn"]="phenotype"
stable.m$Pathway[stable.m$Pathway=="unknown to ecosystem fxn"]="undetermined"
...
##pathogen kingdom -- lump eukaryote and bacteria in with virus in "other microbes"
```{r}
load("log_back_transform.Rdata")
variable_label="pathogen"
load("M.Rdata")
...
##Host kingdom -- lump eukaryote and bacteria and prok. in "microbes". animal blows up, so don't use this version, remove microbes
#https://stats.stackexchange.com/questions/44733/problems-with-mixed-model-simulation
```{r}
load("log_back_transform.Rdata")
variable_label="host"
load("M.Rdata")
euk_bact = subset(M, Host.kingdom == "bacteria" | Host.kingdom == "eukaryote" | Host.kingdom == "prokaryote")
euk_bact$Host.kingdom="microbes"
# euk_bact = subset(M, Host.kingdom == "bacteria" | Host.kingdom == "eukaryote")
# M = euk_bact
M_non = subset(M, Host.kingdom != "bacteria" & Host.kingdom != "eukaryote" & Host.kingdom != "prokaryote")
M = rbind(M_non, euk_bact)
# M = M_non
M = subset(M, !is.na(d))
...
# res.lm.Host.kingdom <- lm(log(d) ~ Host.kingdom-1, weights = 1/d.sampling.variance, data=dat)
# summary(res.lm.Host.kingdom)
# 
# res.lm.Host.kingdom.nolog <- lm(d ~ Host.kingdom-1, weights = 1/d.sampling.variance, data=dat)
# summary(res.lm.Host.kingdom.nolog)


# xtabs(~Host.kingdom, data = M)
# dat = M
dat$d = dat$d+0.000001

res.lme.Host.kingdom <- lme(log(d) ~ Host.kingdom-1, random = ~ 1 | paper.ID, weights = varFixed(~ d.sampling.variance), control=lmeControl(sigma = 1, opt = "optim"), data=dat)


# res.lme.Host.kingdom <- lme(log(d) ~ Host.kingdom-1, random = ~ 1 | paper.ID, weights = varFixed(~ d.sampling.variance), control=lmeControl(sigma = 1), data=dat)
# summary(res.lme.Host.kingdom)

# res.lm.Host.kingdom.lm <- lm(log(d) ~ Host.kingdom-1, weights = 1/d.sampling.variance, data=dat)
# summary(res.lm.Host.kingdom)
...
write.csv(stable.table, "Table.Host.kingdom.microbe_lump.csv", row.names = FALSE)
...
# scale_x_continuous(breaks = pretty_breaks())
plot
ggsave(plot = plot, filename = paste0("Figure.",variable_label,"microbe_lump", ".jpg"))
...
#nlme
##Host kingdom -- lump eukaryote and bacteria and prok. in "microbes"
```{r}
load("log_back_transform.Rdata")
variable_label="pathogen-host"
load("M.Rdata")
i_path = which(M$Pathogen.kingdom=="fungus" | 
                 M$Pathogen.kingdom=="animal" |
                 M$Pathogen.kingdom == "plant")
i_host = which(M$Host.kingdom=="animal" | 
                 M$Host.kingdom=="plant")
i_int = intersect(i_path, i_host)
M = M[i_int,]
M$Host.kingdom=paste(M$Pathogen.kingdom, M$Host.kingdom)
M = subset(M, Host.kingdom !="animal plant")
# euk_bact = subset(M, Host.kingdom == "bacteria" | Host.kingdom == "eukaryote" | Host.kingdom == "prokaryote")
# euk_bact$Host.kingdom="microbes"
# euk_bact = subset(M, Host.kingdom == "bacteria" | Host.kingdom == "eukaryote")
# M = euk_bact
...
#combinations of pathway and measure as one factor
#nlme
```{r}
load("log_back_transform.Rdata")
variable_label="pathway-measure"
load("M.Rdata")
...
M$measure.general[M$measure.general=="primary production"]  ="primary"
M$measure.general[M$measure.general=="secondary production"]  ="secondary"
M$measure.general[M$measure.general=="biogeochemical cycles"]  ="biogeochemical"
# M$
# i_path = which(M$Pathogen.kingdom=="fungus" | 
#                  M$Pathogen.kingdom=="animal" |
#                  M$Pathogen.kingdom == "plant")
...
# i_int = intersect(i_path, i_host)
# M = M[i_int,]
M$Host.kingdom=paste(M$pathway, M$measure.general)
# M = subset(M, Host.kingdom !="animal plant")
# euk_bact = subset(M, Host.kingdom == "bacteria" | Host.kingdom == "eukaryote" | Host.kingdom == "prokaryote")
# euk_bact$Host.kingdom="microbes"
# euk_bact = subset(M, Host.kingdom == "bacteria" | Host.kingdom == "eukaryote")
...
# write.table(rbind(X$observed, X$expected), "test.txt")
#(X <- chisq.test(tbl))
...
```{r}
load("M.Rdata")#source: https://stackoverflow.com/questions/45510421/how-to-make-a-googlevis-multiple-sankey-from-a-data-frame
...
#gvisSankey for plant parasite, with host taxa as beginning
```{r}
load("M.Rdata")
M = subset(M, Pathogen.kingdom=="plant")
...
#gvisSankey for animal parasite, with host taxa as beginning
```{r}
load("M.Rdata")
M = subset(M, Pathogen.kingdom=="animal")
#source: https://stackoverflow.com/questions/45510421/how-to-make-a-googlevis-multiple-sankey-from-a-data-frame
#source on what's possible (but no R here): https://developers.google.com/chart/interactive/docs/gallery/sankey
# source <- sample(c("NorthSrc", "SouthSrc", "EastSrc", "WestSrc"), 100, replace=T)
# mid <- sample(c("NorthMid", "SouthMid", "EastMid", "WestMid"), 100, replace=T)
...
#gvisSankey for fungal parasite, with host taxa as beginning
```{r}
load("M.Rdata")
M = subset(M, Pathogen.kingdom=="fungus")
#source: https://stackoverflow.com/questions/45510421/how-to-make-a-googlevis-multiple-sankey-from-a-data-frame
#source on what's possible (but no R here): https://developers.google.com/chart/interactive/docs/gallery/sankey
# source <- sample(c("NorthSrc", "SouthSrc", "EastSrc", "WestSrc"), 100, replace=T)
# mid <- sample(c("NorthMid", "SouthMid", "EastMid", "WestMid"), 100, replace=T)
...
#gvisSankey for combo of parasiteand host
```{r}
load("M.Rdata")
M = subset(M, Pathogen.kingdom != "not reported")
M = subset(M, Pathogen.kingdom != "multiple")
M = subset(M, Host.kingdom !="multiple")
i = which(M$Pathogen.kingdom == "virus" |
            M$Pathogen.kingdom == "eukaryote" |
            M$Pathogen.kingdom == "bacteria")
M$Pathogen.kingdom[i]="other microbe"

# M$Pathogen.kingdom = subset(M, Pathogen.kingdom != "not reported")
M$pathogen.host = paste(M$Pathogen.kingdom, M$Host.kingdom)
...
#gvisSankey for combo of parasiteand host, simplified
```{r}
load("M.Rdata")
M = subset(M, Pathogen.kingdom != "not reported")
M = subset(M, Pathogen.kingdom != "multiple")
...
i = which(M$Pathogen.kingdom == "virus" |
            M$Pathogen.kingdom == "eukaryote" |
            M$Pathogen.kingdom == "bacteria")
M$Pathogen.kingdom[i]="microbe"

i = which(M$Host.kingdom == "prokaryote" |
            M$Host.kingdom == "eukaryote" |
            M$Host.kingdom == "bacteria")
M$Host.kingdom[i]="microbe"
